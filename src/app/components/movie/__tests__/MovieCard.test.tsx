...
React.useEffect = (callback, deps) => originalUseEffect(() => {
  return callback();
}, deps);
...
jest.mock('framer-motion', () => {
  return {
    motion: {
      div: ({ children, className, onClick, onHoverStart, onHoverEnd }: { 
      ...
      button: ({ children, className, onClick, whileHover, whileTap, 'aria-label': ariaLabel }: { 
        children: React.ReactNode; 
        className?: string; 
        onClick?: () => void;
        whileHover?: any;
...
jest.mock('@/app/redux/hooks', () => {
  ...
  return {
    ...
    selectIsInMyList: (imdbID: string) => (state: any) => {
      return state?.myList?.items?.some((movie: { imdbID: string }) => movie.imdbID === imdbID) || false;
    },
...
describe('MovieCard', () => {
  ...
  const useAppSelectorMock = jest.requireMock('@/app/redux/hooks').useAppSelector;
