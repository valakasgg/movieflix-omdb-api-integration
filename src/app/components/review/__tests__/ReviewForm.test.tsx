...
describe("ReviewForm", () => {
  ...
  it("calls dispatch and onSubmitReview when form is submitted", async () => {
    const { useAppDispatch } = await import("@/app/redux/hooks");
    const dispatchSpy = jest
      .spyOn({ useAppDispatch }, "useAppDispatch")
      .mockImplementation(() => jest.fn());

    renderWithProviders(
      <ReviewForm movieId={mockMovieId} onSubmitReview={mockOnSubmitReview} />
    );
  ...
  it("displays success message after form submission", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Your review has been submitted successfully/i)
      ).toBeInTheDocument();
    });
  });
});
